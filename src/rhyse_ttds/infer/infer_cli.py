...
from rhyse_ttds.infer.utils_infer import (
    cfg_strength,
    cross_fade_duration,
    device,
    fix_duration,
...
parser.add_argument(
    ...
    "--config",
    type=str,
    default=os.path.join(files("rhyse_ttds").joinpath("infer/examples/basic"), "basic.toml"),
...
if "infer/examples/" in ref_audio:
    ref_audio = str(files("rhyse_ttds").joinpath(f"{ref_audio}"))
if "infer/examples/" in gen_file:
    gen_file = str(files("rhyse_ttds").joinpath(f"{gen_file}"))
if "voices" in config:
    for voice in config["voices"]:
        voice_ref_audio = config["voices"][voice]["ref_audio"]
        if "infer/examples/" in voice_ref_audio:
            config["voices"][voice]["ref_audio"] = str(files("rhyse_ttds").joinpath(f"{voice_ref_audio}"))
...
model_cfg = OmegaConf.load(
    args.model_cfg or config.get("model_cfg", str(files("rhyse_ttds").joinpath(f"configs/{model}.yaml")))
)
