...
The cli command `rhyse-ttds_infer-gradio` equals to `python src/rhyse_ttds/infer/infer_gradio.py`, which launches a Gradio APP (web interface) for inference.
...
rhyse-ttds_infer-gradio --inbrowser
...
rhyse-ttds_infer-gradio --root_path "/myapp"
```

Could also be used as a component for larger application:
```python
import gradio as gr
from rhyse_ttds.infer.infer_gradio import app
...
The cli command `rhyse-ttds_infer-cli` equals to `python src/rhyse_ttds/infer/infer_cli.py`, which is a command line tool for inference.
...
rhyse-ttds_infer-cli --model F5TTS_Base --vocoder_name bigvgan --load_vocoder_from_local

# Use custom path checkpoint, e.g.
rhyse-ttds_infer-cli --ckpt_file ckpts/F5TTS_v1_Base/model_1250000.safetensors

# More instructions
rhyse-ttds_infer-cli --help
```

And a `.toml` file would help with more flexible usage.

```bash
rhyse-ttds_infer-cli -c custom.toml
```

For example, you can use `.toml` to pass in variables, refer to `src/rhyse_ttds/infer/examples/basic/basic.toml`:
...
You can also leverage `.toml` file to do multi-style generation, refer to `src/rhyse_ttds/infer/examples/multi/story.toml`.
...
ref_text = ""
```
You should mark the voice with `[main]` `[town]` `[country]` whenever you want to change voice, refer to `src/rhyse_ttds/infer/examples/multi/story.txt`.

## API Usage

```python
from importlib.resources import files
from rhyse_ttds.api import F5TTS

f5tts = F5TTS()
wav, sr, spec = f5tts.infer(
    ref_file=str(files("rhyse_ttds").joinpath("infer/examples/basic/basic_ref_en.wav")),
    ref_text="some call me nature, others call me mother nature.",
    gen_text="""I don't really care what you call me. I've been a silent spectator, watching species evolve, empires rise and fall. But always remember, I am mighty and enduring. Respect me and I'll nurture you; ignore me and you shall face the consequences.""",
    file_wave=str(files("rhyse_ttds").joinpath("../../tests/api_out.wav")),
    file_spec=str(files("rhyse_ttds").joinpath("../../tests/api_out.png")),
    seed=None,
)
```
...
```bash
# Start socket server
python src/rhyse_ttds/socket_server.py
...
python src/rhyse_ttds/socket_client.py
```
...
```bash
python src/rhyse_ttds/infer/speech_edit.py
```
