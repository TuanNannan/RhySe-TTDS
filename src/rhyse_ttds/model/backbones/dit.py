...
from rhyse_ttds.model.modules import (
    AdaLayerNorm_Final,
    ConvNeXtV2Block,
...
class DiT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.severity_embed = nn.Embedding(num_severity_levels, dim)
        ...
        # Initialize severity path as zero so that loading/fine-tuning from F5-TTS
        # starts close to the original model behavior.
        nn.init.constant_(self.severity_mlp[-1].weight, 0)
        nn.init.constant_(self.severity_mlp[-1].bias, 0)
        if text_dim is None:
            text_dim = mel_dim
    ...
    def forward(
        ...
        if severity is not None:
            severity = severity.to(device=x.device, dtype=torch.long)
            sev_emb = self.severity_mlp(self.severity_embed(severity))
            t = t + sev_emb
