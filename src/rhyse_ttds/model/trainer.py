...
from rhyse_ttds.model.dataset import DynamicBatchSampler, collate_fn
from rhyse_ttds.model.utils import default, exists
...
class Trainer:
    ...
    def load_checkpoint(self):
        ...
        if self.is_main:
            ...
            print("Loaded EMA checkpoint with strict=False")
            print("EMA missing keys:", missing)
            print("EMA unexpected keys:", unexpected)
        ...
        if "update" in checkpoint or "step" in checkpoint:
            ...
            if self.is_main:
                print("Loaded pretrained checkpoint with strict=False")
                print("Missing keys:", missing)
                print("Unexpected keys:", unexpected)
            update = 0
    ...
    def train(self, train_dataset: Dataset, num_workers=16, resumable_with_seed: int = None):
        if self.log_samples:
            from rhyse_ttds.infer.utils_infer import cfg_strength, load_vocoder, nfe_step, sway_sampling_coef
        ...
        for epoch in range(skipped_epoch, self.epochs):
            ...
            for batch in current_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    mel_spec = batch["mel"].permute(0, 2, 1)
                    mel_lengths = batch["mel_lengths"]
                    severity = batch.get("severity", None)
