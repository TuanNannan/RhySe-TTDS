...
# 先 debug 一个 speaker。确认没问题后再换成：
SPEAKER_GLOB="torgo_[fm][0-9][0-9]_pinyin"
# SPEAKER_GLOB="torgo_f01_pinyin"
...
OUT_DIR="${PROJECT_ROOT}/exp/f5ttds_chained_cfg_fixedsway_eval"

MODEL_NAME="F5TTS_v1_Base"
VOCODER_NAME="vocos"

# 你之前本地 vocos 路径缺 config.yaml，所以这里先设 0，走 HF 镜像/缓存。
...
# 注意：开启 chained_cfg 后，cfg_strength 不再是主控制量，
# 但保留它是为了兼容普通 CFG 分支。
CFG_STRENGTH=2.0

# fixed：只测试 Chained CFG，不混入 Adaptive Sway。
# severity：后续测试 Chained CFG + Adaptive Sway。
SWAY_SAMPLING_COEF=-1.0

# Chained CFG 必须传 severity，因此这里应使用 speaker。
...
python batch_f5ttds_chained_cfg_eval.py \
