...
# -----------------------------
# Dysarthria-aware adaptive Sway Sampling helpers
# -----------------------------

# TORGO severity mapping used in the current experiment.
# If you later add new speakers or use another split, update this table only.
TORGO_SEVERITY_MAP = {
    "F01": "moderate",
    "F03": "low",
    "F04": "low",
...
def infer_torgo_severity_from_speaker(speaker: str) -> str:
    """Infer TORGO dysarthria severity label from speaker ID."""
    speaker = safe_str(speaker).strip().upper()
    return TORGO_SEVERITY_MAP.get(speaker, "unknown")


def get_severity_adaptive_sway_coef(severity: str, default_coef: float = -1.0) -> float:
    """
    Utterance-level severity-aware adaptive Sway Sampling coefficient.

    The original F5-TTS Sway Sampling formula is kept unchanged in cfm.py.
    This function only changes the coefficient passed into that formula.
    ...
    The value is clipped to [-1.0, 0.0] to avoid an overly aggressive or unstable
    flow-step schedule.
    """
    ...
    coef = mapping.get(severity, float(default_coef))
    return max(-1.0, min(0.0, float(coef)))


def _clip_float(x: float, lo: float = 0.0, hi: float = 1.0) -> float:
...
def estimate_reference_rhythm_score(ref_audio: Path, ref_text: str, args: argparse.Namespace) -> Dict[str, float]:
    ...
    reference speech. This does not perform frame-level ODE step allocation.

    The score is used only to adapt the global Sway coefficient for the current
    utterance. It is intentionally lightweight and interpretable: abnormal pause
    proportion, mean pause duration, local energy fluctuation, and speech-rate
    deviation are combined into a scalar in [0, 1].
    """
    try:
        y_ref, sr = load_audio(ref_audio, args.sr)
        pause = pause_metrics(
            y_ref,
            sr,
            args.pause_top_db,
        ...
        db, _, _ = frame_rms_db(y_ref, sr, args.frame_ms, args.hop_ms)

        pause_ratio = _clip_float(pause.get("pause_ratio", 0.0))
        mean_pause = float(pause.get("mean_pause_duration_sec", 0.0))
        mean_pause_norm = _clip_float(mean_pause / max(float(args.rhythm_max_mean_pause_sec), 1e-8))

        if db.size > 1:
            # Larger frame-to-frame energy fluctuation often corresponds to unstable
            # local dynamics, abrupt pauses, or irregular articulatory transitions.
            energy_delta = float(np.nanmean(np.abs(np.diff(db))))
        else:
            energy_delta = 0.0
        energy_var_norm = _clip_float(energy_delta / max(float(args.rhythm_max_energy_delta_db), 1e-8))

        n_words = count_words(ref_text)
        speech_duration = float(pause.get("speech_duration_sec", 0.0))
        if n_words > 0 and speech_duration > 1e-8:
            speech_rate = float(n_words / speech_duration)
            rate_abn = _clip_float(abs(speech_rate - float(args.rhythm_target_wps)) / max(float(args.rhythm_target_wps), 1e-8))
        else:
            speech_rate = np.nan
            rate_abn = 0.0
        ...
        score = _clip_float(score)
        return {
            "rhythm_score": score,
            "rhythm_pause_ratio": pause_ratio,
            "rhythm_mean_pause_norm": mean_pause_norm,
            "rhythm_energy_var_norm": energy_var_norm,
            "rhythm_speech_rate_wps": float(speech_rate) if np.isfinite(speech_rate) else np.nan,
            "rhythm_rate_abn": rate_abn,
        }
    except Exception as e:
        logging.warning("Failed to estimate rhythm score for %s: %s", ref_audio, e)
        return {
            "rhythm_score": np.nan,
            "rhythm_pause_ratio": np.nan,
            "rhythm_mean_pause_norm": np.nan,
            "rhythm_energy_var_norm": np.nan,
            "rhythm_speech_rate_wps": np.nan,
            "rhythm_rate_abn": np.nan,
        }


def resolve_sway_info_for_row(row: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    """
    Resolve the actual Sway Sampling coefficient and diagnostic information used
    for one generated item.

    Modes:
        fixed:
            Original F5-TTS baseline. Every item uses args.sway_sampling_coef.
        severity:
            F5-TTDS-SevSS. The coefficient is selected according to the TORGO
            severity label inferred from the speaker ID.
        rhythm:
            F5-TTDS-RhySS. The coefficient is first selected by severity and then
            further adjusted using an utterance-level rhythm irregularity score
            estimated from the reference speech.
    """
    mode = safe_str(getattr(args, "adaptive_sway_mode", "fixed")).strip().lower()
    severity = safe_str(row.get("severity", "unknown"))

    if mode == "fixed":
        coef = float(args.sway_sampling_coef)
        return {
    ...
    if mode == "severity":
        base_coef = get_severity_adaptive_sway_coef(severity, default_coef=float(args.sway_sampling_coef))
        return {
            "adaptive_sway_mode": mode,
            "base_sway_coef": float(base_coef),
            "sway_sampling_coef_used": float(base_coef),
            "rhythm_score": np.nan,
            "rhythm_pause_ratio": np.nan,
            "rhythm_mean_pause_norm": np.nan,
            "rhythm_energy_var_norm": np.nan,
    ...
    if mode == "rhythm":
        ...
        ref_text = safe_str(row.get("ref_text", ""))
        rhythm_info = estimate_reference_rhythm_score(ref_audio, ref_text, args)
        rhythm_score = rhythm_info.get("rhythm_score", np.nan)
        score_for_coef = 0.0 if not np.isfinite(rhythm_score) else float(rhythm_score)

        # Higher rhythm irregularity pushes the coefficient further to the left,
        # while the value is clipped to the stable range of Sway Sampling.
        coef = float(base_coef) - float(args.rhythm_sway_gamma) * score_for_coef
        coef = max(float(args.sway_min_coef), min(float(args.sway_max_coef), coef))
        return {
            "adaptive_sway_mode": mode,
            "base_sway_coef": float(base_coef),
            "sway_sampling_coef_used": float(coef),
            **rhythm_info,
        }
...
def resolve_sway_coef_for_row(row: Dict[str, Any], args: argparse.Namespace) -> float:
    """Backward-compatible wrapper."""
    return float(resolve_sway_info_for_row(row, args)["sway_sampling_coef_used"])
...
def build_manifest_from_data_root(
    ...
    for dataset_dir in dataset_dirs:
        ...
        severity = infer_torgo_severity_from_speaker(speaker)
...
- `adaptive_sway_mode`, `base_sway_coef`, and `sway_sampling_coef_used` record the actual sampling setting for each item.
- In `adaptive_sway_mode=rhythm`, `rhythm_score` and its components (`rhythm_pause_ratio`, `rhythm_mean_pause_norm`, `rhythm_energy_var_norm`, `rhythm_speech_rate_wps`, `rhythm_rate_abn`) are estimated from the reference prompt and used to adapt the Sway coefficient.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            "fixed: original F5-TTS fixed Sway coefficient; "
            "severity: severity-aware adaptive Sway Sampling; "
            "rhythm: severity + reference-rhythm-aware adaptive Sway Sampling."
        ),
    )
    p.add_argument("--sway_min_coef", type=float, default=-1.0, help="Lower bound for adaptive Sway coefficient.")
    p.add_argument("--sway_max_coef", type=float, default=0.0, help="Upper bound for adaptive Sway coefficient.")
    p.add_argument("--rhythm_sway_gamma", type=float, default=0.20, help="Strength of rhythm-score adjustment in rhythm mode.")
    p.add_argument("--rhythm_target_wps", type=float, default=2.5, help="Reference normal speech-rate target in words per second.")
    p.add_argument("--rhythm_max_energy_delta_db", type=float, default=10.0, help="Normalization constant for frame-to-frame energy fluctuation.")
    p.add_argument("--rhythm_max_mean_pause_sec", type=float, default=1.0, help="Normalization constant for mean pause duration.")
    p.add_argument("--rhythm_weight_pause", type=float, default=0.40)
    p.add_argument("--rhythm_weight_mean_pause", type=float, default=0.25)
    p.add_argument("--rhythm_weight_energy", type=float, default=0.20)
    p.add_argument("--rhythm_weight_rate", type=float, default=0.15)
...
def main() -> None:
    ...
    logging.info(
        "[INFER] model_name=%s | use_ema=%s | nfe=%s | cfg=%s | sway=%s | adaptive_sway_mode=%s | remove_silence=%s",
        args.model_name,
        args.use_ema,
        args.nfe_step,
        args.cfg_strength,
    ...
    if not bool(args.eval_only):
        ...
        for _, row_s in df.iterrows():
            ...
            sway_info = resolve_sway_info_for_row(row, args)
            local_sway_coef = float(sway_info["sway_sampling_coef_used"])
            rec: Dict[str, Any] = {
            ...
            try:
                if bool(args.skip_existing) and not bool(args.overwrite_existing) and output_wav.exists() and output_wav.stat().st_size > 0:
                    ...
                    logging.info(
                        "Skip existing: %s | speaker=%s | severity=%s | sway=%s | mode=%s | rhythm_score=%s",
                        output_wav,
                        speaker,
                        row.get("severity", ""),
                        local_sway_coef,
                        args.adaptive_sway_mode,
                        sway_info.get("rhythm_score", np.nan),
                    )
                else:
                    ...
                    logging.info(
                        "Generating %s | speaker=%s | severity=%s | ckpt=%s | use_ema=%s | sway=%s | mode=%s | rhythm_score=%s",
                        utt_id,
                        speaker,
                        row.get("severity", ""),
                        ckpt_file,
