...
def resolve_sway_coef_for_row(row: Dict[str, Any], args: argparse.Namespace) -> float:
    """
    Resolve the actual Sway Sampling coefficient used for one generated item.
    ...
    if mode == "fixed":
        return float(args.sway_sampling_coef)
    if mode == "severity":
        return get_severity_adaptive_sway_coef(
            severity=safe_str(row.get("severity", "unknown")),
            default_coef=float(args.sway_sampling_coef),
        )
...
- `adaptive_sway_mode` and `sway_sampling_coef_used` record the actual sampling setting for each item.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="fixed: original F5-TTS fixed Sway coefficient; severity: severity-aware adaptive Sway Sampling.",
    )
        ...
        for _, row_s in df.iterrows():
            ...
            try:
                if bool(args.skip_existing) and not bool(args.overwrite_existing) and output_wav.exists() and output_wav.stat().st_size > 0:
                    ...
                    logging.info(
                        "Skip existing: %s | speaker=%s | severity=%s | sway=%s | mode=%s",
                        output_wav,
                        speaker,
                        row.get("severity", ""),
                        local_sway_coef,
                    ...
                    logging.info(
                        "Generating %s | speaker=%s | severity=%s | ckpt=%s | use_ema=%s | sway=%s | mode=%s",
                        utt_id,
                        speaker,
                        row.get("severity", ""),
                        ckpt_file,
